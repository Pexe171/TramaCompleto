...
export default function NossasEditorias({ editorias }) {
        ...
        {items.map((editoria, index) => {
          ...
          return (
            <div
              ...
              className={`flex justify-center md:justify-start ${isLastOdd ? 'md:col-span-2' : ''}`}
            >
              <Link
                href={editoria.href}
                className="group relative flex w-full max-w-md flex-col overflow-hidden rounded-[2.5rem] border border-black/10 bg-black text-white shadow-[0_25px_60px_rgba(0,0,0,0.4)] transition-transform duration-300 hover:-translate-y-1 md:max-w-none"
              >
                <div
                ...
                <div className="flex flex-col items-start gap-2 px-8 py-7 text-left">
                  <span className="text-[0.7rem] uppercase tracking-[0.28em] text-red-300">Editoria</span>
                  <span className="text-xl font-serif uppercase tracking-[0.14em] text-white">
                    {editoria.title}
                  </span>
                  <span className="text-sm text-slate-200/80">
                    Acesse reportagens, críticas e entrevistas exclusivas da editoria.
                  </span>
                </div>
              </Link>
            </div>
  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto flex w-full max-w-6xl flex-col gap-16 px-6 text-center md:text-left">
        <div className="relative flex flex-col items-center text-center md:items-start md:text-left">
          <span className="font-serif text-[0.75rem] uppercase tracking-[0.35em] text-slate-500 md:text-sm md:tracking-[0.45em]">
            Conheça as nossas
          </span>
          <h2 className="mt-3 text-4xl font-script text-red-600 md:text-5xl">Editorias</h2>
          <p className="mt-4 max-w-2xl text-sm text-slate-600 md:text-base">
            Seções temáticas que organizam o nosso conteúdo para você acompanhar estreias, bastidores e debates sobre cinema e cultura.
          </p>
          <div className="pointer-events-none absolute -right-10 -bottom-6 hidden h-12 w-12 text-red-500/40 md:block">
            <DecorativeStar className="h-full w-full" />
          </div>
        </div>
